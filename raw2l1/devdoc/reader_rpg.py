...
from raw2l1.errors import UnknownFileType
...
def read_brt(filename, accept_localtime=False):
    ...
    with open(filename, 'rb') as f:
        d = f.read()  # ToDo : Better name . E.G. "buf_in"
    ...
    byte_offset += 4
    if data['filecode'] == 666666 or data['filecode']==666667:  # ToDo: local constants at beginning
        anglever = 1
        formatchar_angle = 'f'
    ...
    for n in range(data['n_meas']):
        # ToDO inperformant usage of array access (FIRST LESSON!): Drop for and use append!
        # ToDo: Not using the power of numpy : Strategy: Separate input from processing and storage
        ...
        byte_offset += data['n_freq']*4
        # ToDo: Not using the power of numpy : Strategy: Separate input from processing
