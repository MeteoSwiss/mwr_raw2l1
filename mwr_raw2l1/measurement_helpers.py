...
def scan_endtime_to_time(endtime, n_angles, inttime=40, idletime=1.4):
    ...
    Args
        ...
        idletime : time duration for moving the pointing to the respective scan position in s. The default is 1.4.
...
def scan_to_timeseries_from_aux(blb, brt=None, hkd=None):

    # determine scan duration
    time_scan = blb['time'].values
    n_ele = len(blb['scan_ele'].values)

    endtime2time_params = dict(endtime=blb['time'].values, n_angles=len(blb['scan_ele']))
    if hkd is not None and 'BLscan_active' in hkd:
        time_scan_active = hkd['time'][hkd['BLscan_active'].values == 1]
        # TODO: develop further for infering inttime/caltime/idletime. and replace below placeholders
        endtime2time_params['inttime'] = 40
        endtime2time_params['idletime'] = 1.4
    elif brt is not None:
        pass
        # TODO: infer optional paramters to scan_endtime_to_time from brt (or hkd)
    ...
    time_per_ele = scan_endtime_to_time(**endtime2time_params)
    # TODO: instead of scan_endtime_to_time(**endtime2time_params) call blb = scan_to_timeseries(blb, **endtime2time_params) here

    # TODO: return updated blb
    return blb

def scan_to_timeseries(data, *args, **kwargs):
    """transform scans to time series of spectra and temperatures

    Args:
        data: xarray.Dataset containing the scan observations (BLB)
    ...
    data['time'] = scan_endtime_to_time(data['time'], data['n_ele'], *args, **kwargs)
